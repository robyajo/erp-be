<?php

declare(strict_types=1);

namespace Mitunierp\Blog;

use Mitunierp\PluginManager\Package;
use Mitunierp\PluginManager\PackageServiceProvider;

final class BlogServiceProvider extends PackageServiceProvider
{
    public function configureCustomPackage(Package $package): void
    {
        $package
            ->name('blog')
            ->icon('file-text')
            ->description('Create and manage blog posts, categories, and tags')
            ->latestVersion('1.0.0')
            ->license('MIT')
            ->hasMigrations([
                '2026_06_01_000001_create_blog_categories_table',
                '2026_06_01_000002_create_blog_posts_table',
                '2026_06_01_000003_create_blog_tags_table',
                '2026_06_01_000004_create_blog_post_tag_table',
            ])
            ->runsMigrations()
            ->hasRoutes(['api'])
            ->hasUninstallCommand()
            ->hasInstallCommand(function (\Spatie\LaravelPackageTools\Commands\InstallCommand $cmd) use ($package): void {
                $cmd->startWith(function (\Spatie\LaravelPackageTools\Commands\InstallCommand $command) use ($package): void {
                    $relativePath = $this->getMigrationPath();
                    $command->call('migrate', ['--path' => $relativePath]);
                    $package->updateOrCreate();
                });
            });
    }
}
